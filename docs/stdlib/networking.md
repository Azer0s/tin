...
`Server.accept(max_n)` parks the calling fiber until a datagram arrives, then
returns `(Conn, bool)`. `max_n` is the maximum number of bytes to receive.
